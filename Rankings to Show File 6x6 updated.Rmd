---
title: "2022 Overall 6x6 Projections Updated"
date: "`r Sys.Date()`"
output: 
  html_notebook:
    theme: cerulean
...
```{r load library,message = FALSE,warning=FALSE, results='hide', include=FALSE}
library(readr) #read in csvs
...
Positional Scarcity is determined by [ATC WAR Projections](https://www.fangraphs.com/projections.aspx?pos=all&stats=bat&type=atc&team=0&lg=all&players=0) from 4-10-2022.  
...
pitching_Data_NonAdj_Projections_Total2a = sqldf(
  "
...
hitting_Data_NonAdj_Projections_Total2a = sqldf(
  "
  select a.*, b.Predict_Score
  from
  lookuptable a
...
New_ADP_WAR_H = read_csv('FanGraphs Leaderboard_Hitting_ATC_4_10.csv') %>%
  mutate (
  ) %>% select (PA, playerid, ADP, WAR)


New_ADP_WAR_P = read_csv('FanGraphs Leaderboard_Pitching_ATC_4_10.csv') %>%
  mutate (
  ) %>% select (IP, playerid, ADP, WAR)

pitching_Data_NonAdj_Projections_Total2 = sqldf(
  "
  select a.*, b.WAR, b.ADP,b.IP from
  pitching_Data_NonAdj_Projections_Total2a a
  left join
  New_ADP_WAR_P b
  on a.playerid = b.playerid
  "
...
hitting_Data_NonAdj_Projections_Total2 = sqldf(
  "
  select a.*, b.WAR, b.ADP,b.PA from
  hitting_Data_NonAdj_Projections_Total2a a
  left join
  New_ADP_WAR_H b
  on a.playerid = b.playerid
  "
...
FanGraph_Positions_Batter_Total2 =
hitting_Data_NonAdj_Projections_Total2 %>% 
  mutate(
    Avg_PA = mean(PA , na.rm = T),
    AdjPredict_Score_raw = ifelse(is.na(Predict_Score),NA,Predict_Score*(PA/Avg_PA))
  ) %>% select(-Avg_PA)


Adjustment_factor= (sum(FanGraph_Positions_Batter_Total2$WAR,na.rm = T))/(sum(pitching_Data_NonAdj_Projections_Total2$WAR,na.rm = T))

FanGraph_Positions_Pitcher_Total2 =
pitching_Data_NonAdj_Projections_Total2 %>% 
  mutate(
    Avg_IP = mean(IP , na.rm = T),
    AdjPredict_Score_raw = ifelse(is.na(Predict_Score),NA,Predict_Score*(IP/Avg_IP)*Adjustment_factor)
  ) %>% select(-Avg_IP)


Pitching_for_Overall2 = FanGraph_Positions_Pitcher_Total2 %>% select(Name,Team,Position,ADP,WAR,AdjPredict_Score_raw,Positional_Scarcity)
  
  
Hitting_for_Overall2 =  FanGraph_Positions_Batter_Total2 %>% select(Name,Team,Position,ADP,WAR,AdjPredict_Score_raw,Positional_Scarcity)


FanGraph_Overall2 = rbind(Pitching_for_Overall2,Hitting_for_Overall2)
...
FanGraph_Overall2 %>% 
  mutate(
    WAR_rank = order(order(rank(WAR,ties.method = 'average'),decreasing = TRUE))-sum(is.na(WAR)),
...
ft_dt <- FanGraph_Overall3[1:nrow(FanGraph_Overall3), 1:ncol(FanGraph_Overall3)] %>% 
 arrange((ADP))
...
filter(IP > 19) %>%   
  select (Name,Team,Position,ADP,WAR, WAR_rank,AdjPredict_Score ,AdjPredict_Score_Rank,Ranks_Above_ADP, Positional_Scarcity)  %>% drop_na(WAR)
...
filter(IP > 19 & PositionSP ==1) %>% drop_na(WAR)
...
filter(IP > 19 & PositionRP ==1)  %>% drop_na(WAR)
