...
df_batting_final = as.data.frame(DT_batter2) %>% 
  ...
  select(-(G:AVG),-(OBP:BABIP),-(wOBA:Dol),-(`wRC+`:REW),-(`WPA/LI`),-(wFB:wSF),-BsR,-(Def:wGDP),-(`wCH (pi)`:`wCH/C (pi)`),-(`AVG+`),-(`OBP+`:`BABIP+`),-(H_PA:PH_PA)) 
...
tr_treated_2022 = tr_treated_2021 %>%  select(Worth,one_of(variables_to_keep_2022),starts_with("Team_lev_x_"),playerid) #keep modeled important variables along with team indicator variables

te_treated_2022 = te_treated_2021 %>%  select(Worth,one_of(variables_to_keep_2022),starts_with("Team_lev_x_"),playerid)
...
variables_to_keep_2022_nolag6xb = total_treated_2021_hitting %>%
  filter(Season == 2021) %>% 
select(Worth,one_of(variables_to_keep_2022),starts_with("Team_lev_x_"),playerid)
...
hitting_predictions6xb = as.data.frame(predict(xgb_model_2022,variables_to_keep_2022_nolag6xb))
