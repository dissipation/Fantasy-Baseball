...
Step 2: Re-fit model with reduced variable scope\
    Note from the best tune below `nrounds` - max I set above and `eta` is at the lowest possible value. This could cause potential overfitting issues, but from our Actual vs. Predicted Graph we know this not to be the case.
...
```{r}
Latest_2022_pitchingdata_FP = read_csv("FanGraph_Fantasy_Baseball_Hitting.csv")
...
ft_dt <- Pitching_Data_Adj_Projections[1:nrow(Pitching_Data_Adj_Projections), 1:ncol(Pitching_Data_Adj_Projections)]
